...
def categorize_pdfs(files_data: list):
    ...
    Returns:
      roll60_items: list of dicts with keys: name, bytes, w_pts, h_pts, rotated
      roll91_items: list of dicts
    """
    roll60 = []
    roll91 = []
    ...
    for name, raw_bytes in files_data:
        ...
        # Determine if fits on 60cm roll (either orientation)
        fits_natural = min(w, h) <= ROLL_60_PTS
        fits_rotated = min(h, w) <= ROLL_60_PTS  # same check but track rotation

        # We want the shorter dimension on the roll width axis
        # "width" = cross-roll dimension, "height" = along-roll length
        if w <= ROLL_60_PTS:
            rotated = False
            ...
            # Rotate so height becomes width
            rotated = True
            item = dict(name=name, bytes=trimmed, w_pts=h, h_pts=w,
                        w_cm=h_cm, h_cm=w_cm, rotated=True)
            roll60.append(item)
        else:
            # Doesn't fit 60cm in any orientation → 91cm roll
            item = dict(name=name, bytes=trimmed, w_pts=w, h_pts=h,
                        w_cm=w_cm, h_cm=h_cm, rotated=False)
            roll91.append(item)

    return roll60, roll91
...
with col_info:
    ...
    <div class="card" style="margin-top:0">
        <div class="section-label">Roll Specs</div>
...
if uploaded_files:
    ...
    with st.spinner("Analysing PDFs… trimming bleed, detecting dimensions…"):
        roll60_items, roll91_items = categorize_pdfs(files_data)
...
<div style="text-align:center; margin-top:3rem; font-family:'Space Mono',monospace;
     font-size:.7rem; opacity:.35; letter-spacing:.1em">
    BLUEPRINT ROLL ORGANIZER · PRINT SHOP AUTOMATION
</div>
""", unsafe_allow_html=True)
